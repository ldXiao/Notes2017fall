...
\item $\text{rad}(\sca)\supseteq\sca$
\item $\text{rad}(\text{rad}(\sca))=\text{rad}(\sca)$
\item $\text{rad}(\sca\scb)=\text{rad}(\sca\cap\scb)=\text{rad}(\sca)\cap\text{rad}(\scb)$
\item $\text{rad}(\sca+\scb)=\text{rad}(\text{rad}(\sca)+\text{rad}(\scb))$
\item If $\scp$ is a prime ideal, then $\text{rad}(\scp^k)=\scp$
\end{enumerate}
\end{prop}
...
$$
$$
\cala^n\ni y=y_1 e_1^{(1)}+...+y_n e^{(1)}_n, \ y_i\in \cala
$$
Then 
$$
f(x,y)=\sum_{
...
\end{proof}
\begin{lemma}\label{lem:localization_of_Noetherian}
...
Alternatively, we can also prove this directly. Suppose $K(X_1,...,X_d)$ is finitely generated by $f_1,..,f_n\in K(X_1,..,X_d)$, each $f_i=\frac{g_i}{h_i},\text{ where }g_i, h_i\in K[X_1,...,X_d]$. Set $u:=1+X_1 h_1\cdot\cdot\cdot h_n$
...
\begin{dfn}
In that case call $\scq_j$ the $\scp_j$-\textbf{primary component} of  $\sca$.
\end{dfn}
...
\end{aligned}
$$
For the second identity, we must check that $\forall i\neq j$, $S_{\scp_j}\cap \scq_i\neq \emptyset$ or equivalently $ \scq_i\not\subseteq \scp_j$

Because it is a MPD, we have $\scp_i\not\subseteq \scp_j$ $\exists x\in\scp_i,x\notin\scp_j$ then some $x^n\in\scq_i,$ $\scp_i=\text{rad}(\scq_i)$, $x^n\notin \scp_j$ because $\scp_j$ is prime $\Lrta x\notin \scp_j, \scq_i\not\subseteq \scp_j$.
\end{proof}
...
\item $\text{dim}(S^{-1}\cala)\leq \text{dim}(\cala)$ because for each chain of primes $(\scq_i)\subseteq S^{-1}\cala$ in $S^{-1}\cala$, there exists a chain $(\iota^*\scq_i)\subseteq\cala$ which preserves the proper inclusion.
\end{itemize}
\end{ex}
...
$$ Recall Lemma~\ref{lem:SES_Noetherian} and Lemma~\ref{lem:SES_Artin}, we know $M_0$ Noetherian (Artinian) $\Llrta$ each $M_j/M_{j+1}$ is Noetherian (Artinian)
\end{proof}
\end{lemma}
\end{proof}
\end{lemma}
...
$(iii)$: Elements in $L$ can be written as $\frac{b}{s}, b\in\calb,s\in \calb^{\times}$, $ b\in\calb$ is integral over $\cala$,
$$
...
$$
\scp'/\sca=(\scq'/\scb)\cap(\cala/\sca)=\frac{\scq'\cap\cala}{\scb\cap\cala}=\frac{\scq'\cap\cala}{\sca}\Lrta \scp'=\scq'\cap\cala
$$
...
Now we can summarize what we have know about the dimension theory of finitely generated $k$-domain. $\forall k$ field, $\cala$ a finitely generated $k$-domain, then
\begin{enumerate}[label=(\roman*)]
\item $\text{dim}(\cala)=tr.deg_k(K),$ where $K:=Frac(\cala)$
\item $\text{ht}(\scp)+\text{coht}(\scp)=\text{dim}(\cala),\forall \scp\in \text{Spec}(\cala)$
\item $\text{ht}(\scm)=\text{dim}(\cala),\forall \scm$ maximal, because $\text{coht}(\scm)=0$.
...
\begin{proof}
Maximality of the chain implies: $\scp_0=(0)\Lrta \text{ht}(\scp_0)=0$ and $\scp_r$ maximal and the chain can not be extended $\Lrta \text{coht}(\scp_r)=0$ $\Lrta \text{ht}(\scp_r)=\text{dim}(\cala)$. Also $\forall \text{ht}(\scp_i/\scp_{i-1})=1\Lrta \text{ht}(\scp_i)=\text{ht}(\scp_{i-1})+1$ $\Lrta \text{dim}(\cala)=\text{ht}(\scp_r)-\text{ht}(\scp_0)$.
\end{proof} 

\begin{cor}
(Another proof of Zariski's Lemma~\ref{thm:finite_field_extension} thus the Nullstellensatz~\ref{cor:Nullstellensatz}) Let $K/k$ be field extension s.t. $K$ is finitely generated $k$-algebra, then $K/k$ is finite field extension.
\end{cor}
\begin{proof}
...
f^n+a_1 f^{n-1}+...+a_n=0
$$
Each $v(a_i)\geq 0$ because $a_i\in \cala$ but $v(f)< 0$ get a contradiction as before.
\end{proof}
...
$\sigma:K^\times\lrta G$\\
Define $``\leq \text{ on }G'':$
$x\cala^\times\leq y\cala^\times\Llrta y\in x\cala$ or equivalently $(y x^{-1}\in\cala)$. {\color{red} Notice that in the definition, the ``$y\in x\cala$'' rather than ``$y\in x\cala^\times$'' is of essential importance}

\underline{Claim}: Then ``$\leq $'' is total order. 

Indeed, let $x\cala^\times, y\cala^\times\in G$. Take representatives $x,y\in K^\times$. Then $x/y\in K^\times$.
...
$\sigma(f_1f_2)=\sigma(f_1)\sigma(f_2)=\sigma(f_1)+\sigma(f_2)$: because $\sigma$ is a group morphism and because $G$ is commutative, we can denote the multiplication by $+$.
...
$$
\overline{\cala}=\bigcap_{\substack{\calb:\text{VR, in $K$}\\, \text{with $\cala\subseteq \calb\subseteq K$}}}\calb
$$
\end{thm}
\begin{proof}
...
Also $\varpi\notin\scp^2$, else $(\varpi)=(\varpi^2)\Lrta v(\varpi)=0$ contradiction.
...
\begin{proof}
Write $x=u/v, 0\neq v\in \cala$. It suffices to find $n\geq 0$ s.t., $\scp^n\subseteq(v)$. But $\text{dim}(\cala)=1$, $\scp$ is the only prime that contains $(v)$. $\text{dim}(\cala/(v))=0$. Then, by~\ref{thm:Artin_equiv_Noetherin+dim=0},  $\cala/(v)$ is local Artinian with maximal ideal $\scp/(v)$ but by~\ref{prop:Noetherian_local_Artinian_nilpotent}, $\exists n\geq 0$ s.t. $(\scp/(v))^n=0=\scp^n/(v)$. $\exists n\geq 0$ s.t. $\scp\subseteq(v)$.
\end{proof}
...
\end{lemma} 
\begin{proof}
Just need to check that every condition is preserved by localization. Recall~\ref{lem:localization_of_Noetherian}, (localization of Noetherian ring is Noetherian) and recall that in general $\text{dim}(S^{-1}\cala)\leq \text{dim}(\cala)=1$. If $\text{dim}(\cala)<1$, it has to be a field because $(0)$ is maximal. Otherwise $\text{dim}(S^{-1}\cala)=1$.
...
=0
$$
with each coefficients in $\cala$, so since $\cala$ is normal, deduce that $y\in\cala$, hence that $x=y/s\in\cala_\scp$ (This proof also holds for the general multiplicative set $S$.)
...
$\forall\scm\in Spem(\cala)$, $\cala_\scm$ Noetherian local domain with principal maximal ideal $\Lrta \text{ht}(\scm)=\text{dim}(\cala_\scm)\leq 1$ by Krull principal ideal theorem~\ref{thm:Krull_dimension}.\\
$\text{dim}(\cala)=\sup_\scm\{\text{ht}(\scm)\}= \sup_\scm \text{dim}(\cala_\scm)$. $\cala$ not a field then $\cala\neq 0$. 
\end{proof}
\end{ex}
...
\begin{lemma}\label{lem:Noetherian_dim1_ideal_factors}
$\cala$ Noetherian domain of dimension 1, (every prime  ideal is maximal.) Then every nonzero ideal $\sca\subseteq \cala$ factors uniquely(up to reordering) as a product of primary ideals with distinct radicals.
$$
...
(ii) True if $\cala$ local, because the $\cala$ DVR. 

In general:
$$
\{\scp\text{-primary ideals in }\cala\}\llrta \{\scp\cala_\scp\text{-primary ideals in }\cala_\scp\}
$$
$$
=\{\text{non-zero ideals in $\cala_\scp$}\}=\{\text{powers of $\scp\cala_\scp$}\}
$$

(iii)$\Llta$ (i)+(ii)+Lemma~\ref{lem:Noetherian_dim1_ideal_factors} above. 
\end{proof}
\end{thm}

\begin{dfn}
Let $\cala$ domain. $\cala$ \textbf{fractional ideal} is a submodule $T\subseteq K:=Frac(\cala)$ s.t. $x I\subseteq \cala$ for some $0\neq x\in\cala$.
\end{dfn}
\begin{ex}
If $I=\cala y$ for some $y\in K$, then $I$ is fractional ideal: $y=u/v$, $u,v\in\cala$, $v\neq 0$
So $v I=\cala u\subseteq \cala$. 
\end{ex}

\begin{prop}\label{prop:fractional_ideal_characterizations}
Let $\cala$ be an integral domain and $K:=\text{Frac}(\cala)$. Consider the following conditions on a $\cala$-submodule $M$ of $K$
\begin{enumerate}[label=(\roman*)]
\item There exists ordinary ideal $\sca$ and $\scb\neq (0)$ in $\cala$ such that $M=(\sca:\scb)'$, where $(\sca:\scb)':=\{x\in K, x\scb\subseteq\sca\}$. 

\item There exists an $x\in \cala^\times$ with $x M\subseteq\cala$
\item There exists an $x\in K^\times$ with $x M\subseteq \cala$.
\item $M$ is finitely generated.
\end{enumerate}
Then $(i)\Llrta(ii)\Llrta (iii)\Llta (iv)$, and all 4 characterizations are equivalent if $\cala$ is Noetherian.
\begin{proof}
$(i)\Lrta(ii)$: Take any $x\in \scb$, clearly $x M=x(\sca:\scb)'\subseteq\sca\subseteq\cala$.

$(ii)\Lrta (i)$: $\exists x\in\cala^\times: x M\subseteq\cala$ $x M$ is now an ordinary ideal in $\cala$ and we can write $M=(xM:(x))'$. Indeed we have $(M)\subseteq(xM:(x))'$ and for the converse inclusion, consider $n\in(xM:(x))'\Lrta n\cala x\subseteq xM$ in particular $nx=xm$ for some $m\in M$ and $\cala$ is a domain, $n=m$.

$(ii)\Lrta (iii)$:\checkmark

$(iii)\Lrta (ii)$: If $x=\frac{u}{v}\in K^\times$ with $u, v\in\cala^\times$, $v\cdot\frac{u}{v}M\subseteq\cala\Lrta u M\subseteq\cala$.

$(iv)\Lrta (iii)$: Any finitely generated submodule $M\subseteq K$ is a fractional ideal:
$$
M=\sum_{i=1}^n\cala y_i, y_i=\frac{u_i}{v_i}.
$$
then $v_1,...,v_n M\subseteq \sum\cala u_i\subseteq \cala$.

If we assume in addition $\cala$ is Noetherian, then $(iii)\Lrta (iv)$: $xM\subseteq\cala$ is now an ordinary ideal in Noetherian ring which is finitely generated, assume $x M=\sum_i\cala z_i,\ z_i\in\cala$. Then $M=\sum_i\cala \frac{z_i}{x}$.
\end{proof}
\end{prop}

\begin{dfn} We can define the product of fractional ideals:
$$
I\cdot J:=\left\{\sum_i a_i b_i: a_i \in I, b_i\in J\right\}
$$
and quotient of fractional ideals:
$$
(I:J)=\{x\in K^{\times}:x J\subseteq I\}
$$
Obviously, we have $I\cdot \cala=I$ and $(I:\cala)=I$. We also have $(I:J)\cdot J\subseteq I$.
\end{dfn}

\begin{dfn}
An \textbf{invertible ideal} is an $\cala$-submodule $I\subseteq K$ s.t. $\exists$ submodule $J\subseteq K$ s.t., $IJ=\cala$
\end{dfn}
...
\begin{thm}
$\cala$ Dedekind domain. Then every nonzero fractional ideal is invertible. The group of nonzero fractional ideal is the free group generated by nonzero primes in $\cala$
$$
...
$$
$$
\Lrta I\text{ invertible, because } I\cdot \cala(\varpi^{n-m})=\cala
$$
...
For the general Dedekind domain and a non-zero fractional ideal $I$. Remember that being zero is a local property of $\cala$-module, we have $I_\scp$ nonzero for $\scp$ and it is a $\cala_\scp$-module. By the local case above, we have $I_\scp$ is invertible. Also notice that equality is also a local property:
$$
I\cdot(\cala:I)=\cala\Llta (I\cdot (\cala:I))_\scp\overset{?}{=}\cala_\scp.
$$
It suffices to check the equality
$$
(I\cdot (\cala:I))_\scp=(I_\scp\cdot(\cala_\scp:I_\scp)).
$$
Indeed, one can check that 
$$
\begin{aligned}
&S^{-1} (I\cdot J)\cong S^{-1}I\cdot S^{-1}J\\
&\frac{\sum_i a_i b_i}{s}\mapsto \sum_i\frac{a_i}{s}\frac{b_i}{1}\\
&\frac{\sum_i(\prod_{j\neq i} s_j t_j)a_i b_i}{\prod_i s_i t_i}\mapsfrom\sum_i\frac{a_i}{s_i}\frac{b_i}{t_i}
\end{aligned}
$$ 
...
$$
\begin{aligned}
&S^{-1}(I:J)\subseteq(S^{-1}I:S^{-1}J)\\
\end{aligned}
$$
with equality if $J$ is finitely generated.\\
Given $z\in S^{-1}(I:J)$, write $z=\frac{x}{s}$ with $x\in (I:J), s\in S$. Given $y=\frac{a}{t}\in S^{J}$, $\frac{x}{s}\cdot \frac{a}{t}=\frac{xa}{st}\in S^{-1} I$ because $x J\subseteq I$. 

Conversely, say $J$ is finitely generated by $b_1,..,b_n$ then $S^{-1}J$ is finitely generated by $b_1/1,...,b_n/1$. Given $z\in (S^{-1}I:S^{-1}J)$,  $S^{-1}I\ni z\frac{b_i}{1}$ can be written as $\frac{a_i}{s_i}$ for some $a_i\in I, s_i\in S$. Set $s:\prod_i s_i$ then $sz\cdot b_i\in I$. So $sz\in (I:J)\Lrta z\in S^{-1}(I:J)$.
\end{proof}
\end{thm}
