...
\author{Paul Nelson} %\thanks{Research partially supported by NSF Grant DMS-96-25122 and the Miller Institute for Basic Research in Science.}
\begin{document}
\maketitle
\tableofcontents
\newpage
...
An ideal $\sca \subset R$ with $rad(\sca)=\sca$ is  called \textbf{radical}.
\end{dfn}
...
\begin{dfn}
 The ideal $N:rad((0))$ is called  the \textbf{nil radical} of $\cala$.
\end{dfn}
...
\begin{cor}
Let $\sca\subset  \cala $ be an ideal, then $rad(\sca)$ is the intersection of all prime ideals which contain $\sca$.
\end{cor}
\begin{proof}
consider the homomorphism $\pi :\cala\rightarrow \cala/\sca$
Then $rad(\sca)=\pi^{-1}(\caln_{\cala/\sca})$.
...
\end{proof}
\begin{dfn}
 The \textbf{Jacobson Radical} $Jac(\cala)$ of $\cala$ is the intersection of all maximal ideals in $\cala$.
\end{dfn}
\begin{prop}\label{prop:criterion_jacobson_radical}
We have 
$x\in Jac(\cala)\Longleftrightarrow \forall y \in \cala: 1-xy $ is a unit.
\end{prop}
\begin{proof}
``$\Longrightarrow$'' let $x\in Jac(\cala)$ and $y\in \cala$ s.t. $1-xy$ is not a unit. Then $1-xy\in \scm$ for some maximal ideal 
$\scm \subset \cala$. But $x\in Jac(\cala)\subset \scm,$ hence $1\in \scm$  contradiction.

``$\Longleftarrow$'' let $x  \notin Jac(\cala)$ then $x \notin \scm$ for some maximal ideal $\scm\subset \cala$.
...
 
\begin{prop}
Let $\sca,\scb\subset  \cala$ be ideals s.t. $rad(\sca),rad(\scb)$ are coprime. Then $\sca, \scb$ are coprime.
\end{prop}
\begin{proof}
In fact, we have
$$
...
$$
M/Ker(f)\overset{\bar{f}}{\cong}Im(f).
$$
\end{enumerate}
\end{rmk}
...
(Nakayama's lemma)\\
$M$ finitely generated $\cala$-module, $I\subset Jac(\cala) \text{  the Jacobson radical of $\cala$}$, which is the intersection of all maximal ideals in $\cala$.
...
\begin{cor}
$M$ finitely generated $\cala$-module, $I \subset Jac(\cala), N\subset M$. If $M=IM +N$, then $M=N$.
\end{cor}
\begin{proof}
...
Assume $K$a field $K\subseteq  L$, where $ L$ is also a field. Assume $ L$ is a finitely generated $K$-algebra. Then $ L$ is a finite $K$-algebra $\Llrta$ $ L/K$ is a finite field extension.
\end{thm}
...
\end{cor}
\begin{proof}
Thm $\Lrta$ Cor, Let $\scm\subseteq \cala$ be any maximal ideal, then $ L=\cala/\scm$ is a field.
 \[
\begin{tikzcd}
...
$$
\begin{aligned}
Thm&\Lrta  L/j(\cplx)\text{ is finite field extension}\\
 & \Lrta
...
\end{cor}
\begin{proof}
$K=\cplx,  L=\cplx(x_1,..,x_d)$, then $ L/K$ NOt finite (by thm) $\Lrta  L$ is NOT finitely generated $\cplx$-algebra.

This proof also works when $\cplx$ replaced with any field $K$.

Alternatively, we can also prove this directly, Let $f_1,..,f_n\in K(x_1,..,x_d)$, each $f_i=\frac{g_i}{h_i}\in \cplx[x_1,...,x_d]$. Set $u:=1+x_1 h_1\cdot h_n$
>>>>>>.?
$\Lrta 1/u\notin K[f_1,..,f_n]$ because denominator is coprime to the denominators of the $f_j$.
\end{proof}
...
$\Llrta$ after reordering $x_1,...,x_n,$ $x_1,...,x_d$ algebraically independent over $K$ and $x_{d+1},...,x_n$ is algebraic over $K(x_1,...,x_d)$
>>>>>>>>>>>2
\end{proof}
...
A \textbf{graded module} $M$ over a graded ring $\cala=\oplus_{i\in I}\cala_i$ is a module equipped with a decomposition $M=\oplus_{i\in I } M_i$
 s.t. $\cala_i\cdot M_j\subseteq M_{i+j}$. A \textbf{graded submodule } $M'\subseteq M$ is then a submodule for which $M'=\oplus_{i\in I}(M'\cap M_i)$. A \text{ graded ideal} $\sca$ is  graded submodule of $\cala$ s.t. $\sca=\oplus_i(\sca\cap \cala_i)$.
...
\end{proof}
\begin{dfn}
$\cala\subseteq \calb$, integral extensions, $\scq\in Spec(\calb)$ \textbf{lies over} $\scp\in Spec(\cala)$ iff $\scq\cap\cala=\scp$.
\end{dfn}
\begin{thm}\label{thm:integral_extension_primes_properties}
Let $\cala\subseteq \calb$ integral extensions:
\begin{enumerate}[label=(\roman*)]
\item (lying over): Every  prime $\scp\subset\cala$ has some prime $\scq\subseteq \calb$ lying over it. (equivalently, then map $Spec(\calb)\lrta Spec(\cala)$: $\scq\mapsto \scq\cap \cala$ is surjective.)

\item (Incomparablity) The primes lying over a given prime satisfy no inclusion relations, i.e.,
$$
\left.
\begin{aligned}
&\scq,\scq'\in Spec(\calb)\\
...
\item (Going up) For all $\scp,\scp'\in Spec(\cala),\scq\in Spec(\calb)$ s.t. $\scp\subseteq \scp',\scq\cap\cala=\scp$, \\
$\exists \scq'\in Spec(\calb)$ s.t. $\scq'\supseteq \scq,$ $\scq'\cap \cala=\scp'$
Equivalently, if we start with a chain 
$\scp_1\subseteq ....subseteq \scp_n=\scp\in Spec(\cala)$, then there exists a chain 
$\scq_1\subseteq ...\subseteq \scq_n=\scq\in Spec(\calb)$ 
...
Lying over: Let $\scp\in Spec(\cala)$. $\cala_\scp\subseteq\calb_\scp$ integral, where $\cala_\scp$ is local with maximal $\scm:=\scp\cala_\scp$.
...
>>>>>>4
Consider the prime $\scp'/\scp\subset\cala/\scp\subseteq\calb/\scq$, where the second inclusion is integral extension. By lying over, we can find a prime $Q\subseteq\calb/\scq$ lying over $\scp'\scp$. Then $Q=\scq'/\scq$ for some $\scq'\in Spec(\calb)$
...
\item $\scq\in Spec(\calb)\Lrta g(\scq)\in Spec(\calb), \forall g\in G$???????

\item $\forall \scq,\scq'\in Spec(\calb)$ with $\scq\cap\cala=\scq'\cap\cala$, $\exists g\in G$, s.t. $g(\scq)=\scq'$:
\end{enumerate}
\end{thm}
...
\item 
>>>>>>>>>8
\item Assume first that $L/K$ finite, then $\#G<\infty$. Let $\scq,\scq'\in Spec(\calb)$ with $\scq\cap\cala=\scp=\scq'\cap\cala$. 
...
$Spec(\cala)=\{(x,y): xy=1\}$, by the lying-over properties of integral extensions in Theorem~\ref{thm:integral_extension_primes_properties}, we know there should be a surjective map from $Spec(\cala)$ to $Spec(k[z])$
{\centering
\begin{tikzpicture}
    \begin{axis}
        \addplot [red,thick,domain=0.2:5] ({x},{1/x});
        \addplot [red,thick,domain=-5:-0.2] ({x},{1/x});
        \addplot [blue, thick, domain=-5:5] ({x},{-0.2*x});
    \end{axis}
\end{tikzpicture}
}
...
Let $\cala$ be finitely generated $k$-domain, $\scp\in Spec(\cala)$. Then $ht(\scp)+coht(\scp)=ht(\scp)+dim(\cala/\scp)=dim(\cala)$
\end{thm}
\begin{proof}
...
$$
\scp_0:=\cala_0\cap\scp\in Spec(\cala_0)
$$
that 
...
Choose any $0\neq f\in \scp$. By the key lemma for NN, $\exists z_1,...,z_{n-1}\in\cala=k[x_1,...,x_n]$, s.t. $\cala$ integral over $\cala_0:=k[z_1,...,z_{n-1},z_n]$, where $z_n:=f(x_1,...,x_n)$. Then $tr.deg (Frac(\cala_0))=tr.deg(Frac(\cala))=n$, so $\{z_j\}$ are algebraically independent. Thus $\cala_0$ is again a polynomial ring, hence $\cala_0$ is normal.
...
\item $ht(\scp)+coht(\scp)=dim(\cala),\forall \scp\in Spec(\cala)$
...
\begin{lemma}
Let $\cala\subset K$ be any subring $\scp\in Spec(\cala)$ Set $\mathbb{A}:=$ \{rings $\calb$: $\cala_\scp\subseteq \calb\subseteq K, \scp\calb\neq \calb$\}
>>>>>>>>?
\begin{proof}
$(i)\Lrta (ii)$ $\scm\cap \cala=$
>>>>>>1

Choose any maximal ideal $\scm$ with $\scp\calb\subseteq \scm\subset \calb$. It exists because $\scp\calb\neq \calb$. Then $\scp\calb_\scm\subseteq \scm\calb_\scm\neq \calb_\scm$, because $\calb_\scm\neq \{0\}$. By maximality of $\calb$, must have $\calb_\scm=\calb$. (Else $\calb_\scm\supsetneq \calb, \scp\calb_\scm\neq \calb_\scm$) $\Lrta \calb$ is local with maximal ideal $\scm$. Also $rad(\scp\calb)=\scm$. Indeed, we have that for any prime that contains $\scp\calb$, the ring $\calb$ is local with maximal ideal $\scm$. Thus there is only one such prime.
...
Want: $1/x\in \calb$. Thus by ``lying over'', $\exists \scn\in Spec(\calb[\frac{1}{x}])$ with $\scn\cap \calb=\scm$.
...
\begin{tikzcd}
\boxed{Normal} \arrow[rr, "(*)"', bend right] &  & \boxed{DVR} \arrow[dd, "\checkmark", bend left] \arrow[ll, "\checkmark "', bend right] &  &  \\
 &  &  &  &  \\
 &  & \boxed{PID^+} \arrow[d] \arrow[uu, "(?)", bend left] &  &  \\
 &  & \boxed{PID} \arrow[d] &  &  \\
 &  & \boxed{PID^-} \arrow[rr, "def"', leftrightarrow] \arrow[uu, "(?)", bend left] &  & \boxed{Regular} 
...
\begin{tikzcd}
\boxed{DVR} \arrow[d, "\checkmark"] \arrow[r, "\checkmark", bend left] & \boxed{PID^+} \arrow[l, "\checkmark", bend left] \arrow[d, "\checkmark"] \\
\boxed{Normal} \arrow[u, "(I)", bend left] & \boxed{PID} \arrow[d, "\checkmark"] \\
\boxed{Regular} \arrow[r, leftrightarrow, "\checkmark"] & \boxed{PID^-} \arrow[uu, "(II)"', bend right]
\end{tikzcd}
\begin{proof}
...
What is $Spec(\intg[1/6])$

What is the $Ker(\cala\lrta \cala_f)$
