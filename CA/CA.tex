...
\begin{enumerate}[label=(\alph*)]
\item $\scp\cup S\neq \emptyset \Lrta ]iota)*\scq=(1)$
\item $\scp\cap S=\emptyset$($\iota_*\scp$ is prime)$\Lrta \iota_*\scq$ is $\iota_* \scp$-primary.
\end{enumerate}
\begin{proof}
\begin{enumerate}[label=(\alph*)]
\item 
...
Let $\frac{x}{s},\frac{y}{t}\in S^{-1}\cala$, Suppose $(\frac{x}{s})\frac{y}{t}$
>>>>>>>>>>>>1
\end{enumerate}
\end{proof}
\end{lemma}
