...
\title{\bf Personal Notes for Commutative Algebra by Prof. Nelson}
...
 
\begin{prop}
Let $\sca,\scb\subset  A$ be ideals s.t. $rad(\sca),rad(\scb)$ are coprime. Then $\sca, \scb$ are coprime.
\end{prop}
\begin{proof}
In fact, we have
$$
...
>>>>>>1 (Int the memory not in onenotes)
...
-ax^2-zx+1=0
$$
$\Lrta \cala[x,z]$ is integral over $k[z], \forall a\in k, a\neq 0$.
...
\begin{lemma}
In the context of Baby case, write $f=f_n+...f_0$. where $f_i$ homogeneous of degree $i$ and $f_n\neq 0$. If $Y-aX\not | f_n$, $a\in k$. Then $\cala$ is integral over $k[y-ax]$
\end{lemma}
\begin{cor}
If $k$ is infinite, then Baby case holds.
\end{cor}
\begin{proof}
By the above lemma, it suffices to find $a\in k$ s.t. $Y-aX\not | f_n$. Over $\overline{k}$, $f_n=\prod_{i=1...,n} (\alpha_i x-\beta_i Y)$ $\alpha_i,\beta_i\in \overline{k}$ not both zero. 
...
& \text{where each power equals} x_n^{s_j\alpha_j}+(...) \text{ lower powers with coefficients in $z$}\\
&= x_n^{l(\alpha)}+(...),\ \ \ l(\alpha):=s_1\alpha_1+...+s_{n-1}\alpha_{n-1}+\alpha_n, \text{ where $(...)$ are of lower powers in $x_n$ with coefficients in $k[z_1,..,z_{n-1}]$}
\end{aligned}
$$
>>>>>>>>>>4
...
\begin{thm}
Let $\cala$ finitely generated $k$-domain, $\scp\in Spec(\cala)$. Then $ht(\scp)+coht(\scp)=ht(\scp)+dim(\cala/\scp)=dim(\cala)$
\end{thm}
\begin{proof}
...
$\cala_0$ is UFD $\Lrta$ $\cala_0$ is normal. $\cala$ is a domain., $\cala_0\subseteq \cala$ is an integral extension. So by the result form last lecture, we have with 
$$
...
If $\scp=(0)$, then OK: $ht(\scp)=0$, $coht(\scp)=dim(\cala)$. So we may assume $\scp\neq (0)$.
...
Also $coht(\overline{\scp})=dim(\overline{\cala})=coht(\overline{\scp})$, $\dim(\overline{\cala})=n-q=dim(\cala)-1$.

In general $ht(\scp)+coht(\scp)\leq dim(\cala)$ holds trivially. so we just need to show $ht(\scp)+coht(\scp)\geq dim(\cala)$. where $ht(\scp)\geq ht(\overline{\scp})+1$, $coht(\scp)=coht(\overline{\scp})$ and $dim(\cala)=dim(\overline{\cala})+1$.

So we can argue by induction on $n$ in the special case $x_n\in \scp$. We now reduce the general case to this one.

Choose any $0\neq f\in \scp$. By the key lemma for NN, $\exists z_1,...,z_{n-1}\in\cala=k=k[x_1,...,x_n]$, s.t. $\cala$ $\cala$ integral over $\cala_0:=k[z_1,...,z_{n-1},z_n]$, where $z_n:=f$. Then $tr.deg (\cala_0)=tr.deg(\cala)=n$, so $\{z_j\}$ are algebraically independent. Thus $\cala_0$ is again a polynomial ting, hence $\cala_0$ is normal.

By arguing as in the beginning of the proof, we have with $\scp_0:=\scp\cap\cala_0$ that $dim(\cala_0)=dim(\cala)$, $coht(\scp_0)=coht(\scp)$, $ht(\scp_0)=ht(\scp)$. Si iit suffices to show that $coht(\scp_0)+ht(\scp_0)\geq dim(\cala_0)$. 
