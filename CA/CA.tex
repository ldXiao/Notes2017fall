...
\begin{dfn}
 The ideal $N:\text{rad}((0))$ is called  the \textbf{nil radical} of $\cala$.
\end{dfn}
...
$$
Equivalently, we can define: an ideal $\scq$ is primary if, whenever $xy\in\scq$, we have either $x\in\scq$ or $y\in \text{rad}(\scq)$
\begin{dfn}
...
\textbf{subclaim}: $\text{Ann}(y)\cap(y)=0$.\\
 ...
 Proof of Claim2: Consider the complement $S^c=$\{idaels in $\cala$ that are not finite intersections of irreducible ideals\}. \underline{Want}:$S^c=\emptyset$. If not, then it contains a maximal element $\sca$. Claim $\sca\neq (1)$, because $\sca$ not irreducible. 
...
If $\scm$ maximal ideal, $\scq$ any ideal, and $\scm=\text{rad}(\scq)$, then $\scq$ is primary.
\begin{proof}
...
\end{enumerate}
\begin{dfn}
If we can't do (a) or (b), we call the resulting decomposition \textbf{minimal}. Let $\sca$ ideal, we define $Ass(\sca):=\{\text{prime ideals of the form $\text{rad}(\sca:x)$ for some $x\in\cala$}\}$ to be \textbf{the set of associated ideals of $\sca$}.(recall $y\in(\sca:x)\Llrta y$ maps $x$ into $\sca\Llrta yx\in\sca$ )
\end{dfn}
...
\begin{proof}
$\{p_j\}\supseteq Ass(\sca)$. Let $x\in\cala$ s.t. $\text{rad}(\sca:x)=\scp$ is prime. want $\scp=$some $\scp+j$. $\text{rad}(\sca:x)=\cap \text{rad}(\scq_j:x)=\cap_{x\notin\scq_j}\scp_j\Lrta\scp=$some $\scq_j$
>>>>>>>>1
\end{proof}
...
\textbf{Note that} $\text{rad}(\sca)=\cap\scp_j$ if $\sca=\cap\scq_j$ is a  MPD.
We want to define 
...
Recall: give $j$, we can find $y$ s.t. $\text{rad}(\sca:y)=\scp_j$. Necessarily, $y\notin\sca$. So if $x\in \scp_j$, then $x\in  \text{rad}(\sca:y)\subseteq Z(\sca)$ because $\text{rad}(\sca:y)=\cap_{j:\scq_j\not\ni y}\scp_j$ 
\end{proof}
\end{prop}
...
\item Suppose $\scp \cap S=\emptyset$. Then $\text{rad}(\iota_*(\scq))=\iota_*(\text{rad}(\scq))=\iota_*\scp$.
...
\begin{proof}
Last time $\text{rad}(\iota_*\scq)=\iota_*\text{rad}(\scq)=\iota_*\scp$.\\
...
The $\Llta$ direction of (c) is trivial. For the ``$\Lrta$'' direction of (c) Suppose $\exists s\in S\cap\scp$. Since $\scp=\text{rad}(\scq)$, $\exists n\geq 1 $ s.t. $s^n\in \scq$. $S$ multiplicative closed $\Lrta s^n\in S$. Thus $s\in S\cap \scq$. So $S\cap \scp\neq \emptyset \Lrta S\cap\scq\neq \emptyset$
...
Let $\cala$ Artin. Then every prime in $\cala$ is maximal $\cala$ has only finitely many primes, hence the Jacobson radical $\text{Jac}(\cala)=Nil(\cala)$
\end{lemma}
\begin{proof}
...
\begin{proof}
Let $\mathcal{J}:= Nil(\cala)=\text{Jac}(\cala)$ by the lemma. Consider $\mathcal{J}\supseteq \mathcal{J}^2\supseteq \mathcal{J}^3\supseteq ...$ $\cala$ is Artin $\Lrta \mathcal{J}^n=\mathcal{J}^{n+1}$ for some $n$, Want $\mathcal{J}^n=(0)$\\
...
\end{aligned}
$$
Consider $(\scp_1,...,\scp_r)^N\subseteq (\scp_1\cap...\cap\scp_r)^N\subseteq (0)$, where $(\scp_1\cap...\cap \scp_r)=Nil(\cala)=\text{rad}(0)$ and we can conclude the last inclusion by Lemma~\ref{lem:ideal_radical_Noetherian}
\end{proof}
\begin{dfn}
...
$\Lrta$ $\scm=\text{Jac}(\cala)=Nil(\cala)$\\
