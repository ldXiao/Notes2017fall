...
\begin{prop}$S$ is a multiplicative set in a ring $\cala$, then for the canonical morphism $\iota:\cala\lrta S^{-1}\cala$:
\begin{enumerate}[label=(\alph*)]
\item $\iota^*:\text{Spec}(S^{-1}\cala))\lrta\{\scp\in \text{Spec}(\cala)|\scp\cap S=\emptyset\}$ is a bijection.
...
$S\subseteq \cala$ then we can define  $S^{-1}\cala$.
Also we define \textbf{localization of modules:} $S^{-1}M\cong S^{-1}\cala\otimes_\cala M$. The localization of module defines a functor $S^{-1}$:
