...
\end{tikzcd}
\]
Let $Q'\subseteq\calc$ lie over $\scp'$, it exists by Lying-Over.\\
Let $Q_1\subseteq\calc$ lie over $\scp$, with $Q_1\supseteq Q'$. It exists by Going-Up.\\
Let $Q\subseteq\calc$ lie over $\scq$, it exists by Lying over. 

Notice that both $Q$ and $Q_1$ lie over $\scp$, by Theorem~\ref{thm:normal_domain_normal_Galois}, $\exists g\in\text{Aut}(L/K)$ s.t. $g(Q_1)=Q$.
