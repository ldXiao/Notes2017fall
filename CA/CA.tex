...
\begin{dfn}
$Ass(\sca)\ni \scp$ is either \\
\textbf{minimal/isolated} is $\scp$ is a minimal element of $Ass(\sca)$ or\\
...
>>>>>>>>>>>>>1
%$\Llrta \sum(M_i\cap M')\subseteq M'\sum_\alpha \cala x_\alpha\subseteq=\sum_\alpha M_{i(\alpha)}\cap M' \subseteq \sum_i M_i\cap M'$
\item $M'\subseteq M$ graded, finitely generated.\\
Similar.
%$\Lrta\exists x_1,..,x_n\in M'$ s.t. $M'=\sum^n_{\alpha=1}\cala x_\alpha\subseteq \sum^n_{\alpha=1}\sum_i \cala y_{\alpha_i}\subseteq M'\Lrta$ $M'$ is generated by $\{y_\alpha:y_{\alpha_i}\neq 0\}$ a finite set.
\end{enumerate}
\end{proof}
...
>>>>>>>1 recall
Now we come back to the proof of Krull-intersectoin theorem. Last time we proveed the following are equivalent $(\cala-\text{Noetherian}), \sca\text{ any ideal}, (M_i)_{i\geq 0}:\sca-\text{filtration}, $ $\sca M_i\subseteq M_{i+1}$\\
$$
...
Another version:
\begin{lemma}\label{lem_5'}
$0\neq f(\text{ non-unit })\in\cala:$ Noetherian integral domain. Then any minimal prime $\scp_0$ of $(f)$ satisfies $ht(\scp_0)= 1$\\
$\scp_0\supseteq (f)$ minimal for this property ``$\scp_0\in Ass'((f))$''
\end{lemma}

Reduce Lemma~\ref{lem_5'} to Lemma~\ref{lem_5}, we may assume that $\cala$ is local with $\scp_0$ the maximal ideal:
\begin{itemize}
\item replace $\cala$ by $\cala_{\scp_0}$,\ \  $\iota:\cala\lrta \cala_{\scp_0}$
\item replace $\scp_0$ by $\iota_*(\scp_0)$
\item replace $f$ by $f/1=\iota(f)$
\end{itemize}

Then there is a bijection between the set of $\{\text{primes $\scp\subseteq \scp_0$}\}$ and \{\text{ primes of $\cala_{\scp_0}$}\}, and a bijection between: $\{\text{primes $\scp\ni f$}\}$ and $\{\text{ primes of }\cala_{\scp_0}|\scp\ni f/1\}$
>>>>>>>>3
...
$\Lrta\scp^{(i)}=\scp^{(k)}\forall i\geq k$ $\Lrta$ $\scp^k\subseteq \scp^{(k)}=\cap_i\scp^{(i)}=$ (by some Cor in this lecure) $Ker(\cala\lrta \cala_\scp)=\{0\}$ the last equality because $\cala$ is a domain.
